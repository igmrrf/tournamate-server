...
namespace UseCase.Commands.TournamentCommand
{
    ...
    {
        public record SaveToDraftCommand(Guid TournamentId) : IRequest;

        public class Handler(ITournamentRepository tournamentRepository, IUnitOfWork unitOfWork, IUserService userService) : IRequestHandler<SaveToDraftCommand>
        {
            public async Task Handle(SaveToDraftCommand command, CancellationToken cancellationToken)
            {
                ...
                getTournament.SaveToDraft(getTournament);
